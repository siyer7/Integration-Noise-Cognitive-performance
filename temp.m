...
% % fit line
% [alpha, beta, sz] = splitapply(@lineFit, data.confidence, data.accuracy, ocGroup); 
% oct2 = table(alpha, beta, subjID, condID); % create table
%[alpha, beta, sz] = splitapply(@lineFit, data.confidence, data.accuracy, ocGroup);

figure(7)

f4 = gramm('x', data.accuracy, 'y', data.confidence);%, 'color', data.condition);
%f4.facet_grid(data.condition);

%f4.facet_wrap(data.condition);
%f4.geom_abline('intercept', alpha, 'slope', beta, 'style', 'k--');
%f3.geom_point('intercept', .75, 'slope', 1/4, 'style', 'k--');
%f4.stat_glm('distribution', 'normal');
%f4.stat_summary('type', 'bootci', 'geom', 'point');
%f4.stat_summary('type', 'bootci', 'geom', 'black_errorbar');

f4.set_title("                              Confidence vs Accuracy by Condition");
f4.set_names('x', 'accuracy', 'y', 'confidence', 'column', 'Subject', 'row', '');

%going to add the means instead of subtract because they are stored with
%the opposite sign
abs(mean(data.trueOrientaions) + data.orientationMean)

%f4.axe_property('xlim', [.5 1])
f4.draw();

% data.accuracy
% [N,edges,bins] = histcounts(data.accuracy,10);
% N
% 5;
% bins;
% 5;
% meanAcc = accumarray(bins(:),data.accuracy,[],@mean);
% meanConf = accumarray(bins(:),data.confidence,[],@mean);
% 
% f5=gramm('x',data.accuracy,'color',data.condition);
% 
% f5.stat_qq();
% f5.stat_bin('nbins',20);
% 
% f5.set_title('stat_bin()');

% f5.set_names('x','accuracy','y','confidence');
% 
% f5.set_title('Conf vs acc');
% 
% f5.draw();
...
% fit psychometric curve
function [t1, t2, sz] = lineFit(x, y)
    theta = glmfit(x, y, 'normal', 'link', 'identity');
    t1 = theta(1);
    t2 = theta(2);
